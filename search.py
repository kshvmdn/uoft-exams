def search(all_courses, student_courses, last_name):
    ...
    def name_in_section(section, ln=last_name):
        if '-' in section:
            n = section.split(' - ')  # e.g. section = 'K - SH'
            # some sections may have lecture code too, e.g. 'L0201  K - SH'
    ...
    def lecture_in_section(student_course, section):
        ...
        section has a lecture code if:
          - it has a name range (dash) w. a double space, e.g. 'L0201  K - SH'
          - it has no name range (no dash), e.g. 'L0201'
    ...
    return ([course for course, student_course in
             filter(is_match, [(c, sc)
                               for sc in student_courses
                               for c in all_courses])] or None)
